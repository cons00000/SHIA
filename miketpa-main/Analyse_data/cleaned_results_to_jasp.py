...
UTILISATION :
    python cleaned_results_to_jasp.py --input ./logs --output resultats_jasp.csv --q1 Q1_questionnaire_initial.csv --q2 Q2_journal_quotidien_de_sommeil.csv --q3 Q3_post_interaction.csv

    python cleaned_results_to_jasp.py --compute-fit resultats_jasp.csv
"""
import os
import re
import csv
...
# Colonnes texte libre : toutes supprimées — Q3_Delai_changements et Q3_Accord_journal_T1
# sont désormais absorbés dans Q3_Score_Engagement_Concret uniquement.
Q3_TEXT_COLS = {}
...
def aggregate_session(participant_id, agent_profile, rows, filename,
    ...
    # Deltas principaux (VD3)
    for suffix in ['Duree_sommeil', 'Qualite_sommeil']:
        col_name = f'Q2_Delta_{suffix}'
        result[col_name] = q2_entry.get(col_name, '')
    # Deltas secondaires regroupés en VD4
    vd4_vals = []
    for suffix in ['Pct_nuits_avec_reveil', 'Humeur_reveil', 'Forme_physique',
                   'Difficulte_endormissement', 'Stress_soir']:
        v = q2_entry.get(f'Q2_Delta_{suffix}')
        try:
            vd4_vals.append(float(str(v).replace(',', '.'))) if v not in ('', None) else None
        except (ValueError, TypeError):
            pass
    result['VD4_Score_Bien_Etre_Delta'] = round(sum(vd4_vals) / len(vd4_vals), 3) if vd4_vals else ''

    # VD3 scores synthétiques de qualité de sommeil
    def _q2_float(key):
        v = q2_entry.get(key)
        try:
    ...
    def _q2_score_sommeil(label):
        ...
        qualite = _q2_float(f'Q2_{label}_Qualite_sommeil_moy')          # 1–5
        duree   = _q2_float(f'Q2_{label}_Duree_sommeil_moy')            # heures
        diff    = _q2_float(f'Q2_{label}_Difficulte_endormissement_moy') # 1–5 (inversé)
        parts = []
        if qualite is not None:
            parts.append(_norm(qualite, 1, 5))
        if duree is not None:
            parts.append(_norm(duree, 4, 9))       # plage raisonnable : 4h–9h
        if diff is not None:
            parts.append(_norm(5 - diff + 1, 1, 5))  # inversion
        return round(sum(parts) / len(parts), 3) if parts else None

    result['VD3_Score_Sommeil_T0'] = _q2_score_sommeil('T0')
    result['VD3_Score_Sommeil_T1'] = _q2_score_sommeil('T1')
